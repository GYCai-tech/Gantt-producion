...
def test_navegar_a_manana_conserva_el_bono_activo_de_hoy(monkeypatch):
    ...
    monkeypatch.setattr(lecturas, 'leer_lineas', lambda *args: [])
    activa = dict(bono(), idlinea=1, idoperacion=0, abierta=True,
                  fecha=AHORA, inicio=AHORA.replace(hour=11), fin=None)
    monkeypatch.setattr(lecturas, 'leer_abiertas', lambda *args: [activa])
    monkeypatch.setattr(cache, 'cargar_estimaciones', lambda: ({(1, 10): (1, 5), (2, 10): (1, 1)}, MEDIAS, MONTAJES_TEST))
    monkeypatch.setattr(lecturas, 'leer_avance', lambda *args: {
        (1, 10): {'minutos': 70, 'min_produccion': 10, 'min_montaje': 60,
                  'piezas': 10, 'operarios': 1, 'montando': 1},
    })
    monkeypatch.setattr(lecturas, 'leer_cola', lambda: [bono(2)])
    for vista in ('empleado', 'maquina'):
        items = produccion.calcular_items(vista=vista, desde=datetime(2026, 9, 8),
...
def test_abiertas_usa_hora_real_y_no_fecha_de_grabacion(monkeypatch):
    ...
    def leer(query, params):
        ...
        return []
    monkeypatch.setattr(lecturas, 'consultar', leer)
    assert lecturas.leer_abiertas(AHORA) == []
...
def test_fichaje_antiguo_sin_cerrar_no_se_proyecta_como_trabajo_actual(monkeypatch):
    ...
    monkeypatch.setattr(lecturas, 'leer_lineas', lambda *args: [antigua])
    monkeypatch.setattr(lecturas, 'leer_abiertas', lambda *args: [])
    monkeypatch.setattr(cache, 'cargar_estimaciones', lambda: ({(1, 10): (1, 5)}, MEDIAS, MONTAJES_TEST))
    monkeypatch.setattr(lecturas, 'leer_avance', lambda *args: {})
    monkeypatch.setattr(lecturas, 'leer_cola', lambda: [])
    items = produccion.calcular_items(vista='empleado', desde=datetime(2026, 9, 4),
                          hasta=datetime(2026, 9, 9))
...
def test_navegar_al_futuro_no_devuelve_barras_que_ya_terminaron(monkeypatch):
    ...
    activa = dict(bono(cantidad=10), idlinea=1, idoperacion=0, abierta=True,
                  fecha=AHORA, inicio=AHORA.replace(hour=11), fin=None)
    monkeypatch.setattr(lecturas, 'leer_abiertas', lambda *args: [activa])
    monkeypatch.setattr(cache, 'cargar_estimaciones', lambda: ({(1, 10): (1, 1)}, MEDIAS, MONTAJES_TEST))
    monkeypatch.setattr(lecturas, 'leer_avance', lambda *args: {
        (1, 10): {'minutos': 1, 'min_produccion': 1, 'min_montaje': 0,
                  'piezas': 1, 'operarios': 1, 'montando': 1},
    })
    monkeypatch.setattr(lecturas, 'leer_cola', lambda: [])
    assert produccion.calcular_items(vista='empleado', desde=datetime(2026, 9, 8),
                         hasta=datetime(2026, 9, 9)) == []
...
def test_el_area_de_una_barra_es_la_de_su_bono_no_la_de_quien_lo_hace(monkeypatch):
    ...
    abierta = dict(bono(), idlinea=1, idoperacion=0, abierta=True, area='CHAPA',
                   fecha=AHORA, inicio=AHORA.replace(hour=11), fin=None)
    monkeypatch.setattr(lecturas, 'leer_lineas', lambda *a: [])
    monkeypatch.setattr(lecturas, 'leer_abiertas', lambda *a: [abierta])
    monkeypatch.setattr(cache, 'cargar_estimaciones', lambda: ({(1, 10): (1, 5)}, MEDIAS, MONTAJES_TEST))
    monkeypatch.setattr(lecturas, 'leer_avance', lambda *a: {
        (1, 10): {'minutos': 10, 'min_produccion': 10, 'min_montaje': 0,
                  'piezas': 99, 'operarios': 1, 'montando': 1},     # casi terminada: deja hueco hoy
    })
    monkeypatch.setattr(lecturas, 'leer_cola', lambda: [dict(bono(2), cantidad=5, area='ESTRUCTURAS')])

    items = produccion.calcular_items(vista='empleado')
    real = next(i for i in items if i['tipo'] == 'real')
...
def _montar(monkeypatch, lineas, abiertas=()):
    monkeypatch.setattr(produccion, 'datetime', Reloj)
    monkeypatch.setattr(produccion, 'date', Fecha)
    monkeypatch.setattr(lecturas, 'leer_lineas', lambda *a: list(lineas))
    monkeypatch.setattr(lecturas, 'leer_abiertas', lambda *a: list(abiertas))
    monkeypatch.setattr(cache, 'cargar_estimaciones', lambda: ({(1, 10): (1, 5)}, MEDIAS, MONTAJES_TEST))
    monkeypatch.setattr(lecturas, 'leer_avance', lambda *a: {})
    monkeypatch.setattr(lecturas, 'leer_cola', lambda: [])
...
def test_un_trocito_lejano_no_se_funde(monkeypatch):
    ...
    assert len(produccion.calcular_items(vista='empleado')) == 2
