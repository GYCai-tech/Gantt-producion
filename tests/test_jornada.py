...
def test_dentro_de_jornada_no_mueve_nada():
    ...
    assert siguiente_hueco(t) == t


def test_antes_de_abrir_espera_a_las_siete():
    assert siguiente_hueco(VIERNES.replace(hour=5)) == VIERNES.replace(hour=7)


def test_despues_de_cerrar_salta_al_dia_siguiente():
    assert siguiente_hueco(VIERNES.replace(hour=15)) == LUNES.replace(hour=7)
    assert siguiente_hueco(VIERNES.replace(hour=21)) == LUNES.replace(hour=7)


def test_el_fin_de_semana_se_salta():
    sabado = datetime(2026, 9, 5, 9)
    domingo = datetime(2026, 9, 6, 9)
    assert siguiente_hueco(sabado) == LUNES.replace(hour=7)
    assert siguiente_hueco(domingo) == LUNES.replace(hour=7)


def test_lo_que_cabe_en_el_dia_se_queda_en_el_dia():
    assert sumar_laborables(VIERNES.replace(hour=9), 90) == VIERNES.replace(hour=10, minute=30)
...
def test_lo_que_no_cabe_continua_el_siguiente_dia_laborable():
    ...
    assert sumar_laborables(VIERNES.replace(hour=14), 120) == LUNES.replace(hour=8)
...
def test_una_jornada_entera_son_480_minutos():
    ...
    assert sumar_laborables(VIERNES.replace(hour=7), 480) == VIERNES.replace(hour=15)
    # 481 ya no cabe: el minuto sobrante abre el lunes.
    assert sumar_laborables(VIERNES.replace(hour=7), 481) == LUNES.replace(hour=7, minute=1)


def test_arrancar_fuera_de_hora_no_regala_tiempo():
    # A las 21:00 del viernes no queda jornada: empieza el lunes a las 7.
    assert sumar_laborables(VIERNES.replace(hour=21), 60) == LUNES.replace(hour=8)


def test_un_bono_largo_cruza_varios_dias():
    # 960 min = dos jornadas completas: viernes entero + lunes entero.
    assert sumar_laborables(VIERNES.replace(hour=7), 960) == LUNES.replace(hour=15)


def test_medicion_y_suma_laborable_coinciden_al_cruzar_un_fin_de_semana():
    from app.calculos.calendario import minutos_laborables_entre
    inicio = VIERNES.replace(hour=14)
    fin = sumar_laborables(inicio, 600)
    assert minutos_laborables_entre(inicio, fin) == 600
