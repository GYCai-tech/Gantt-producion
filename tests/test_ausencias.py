...
from app.erp import cache, cliente, consultas, lecturas
...
def fingir_erp(monkeypatch, filas):
    monkeypatch.setattr(lecturas, "consultar", lambda *a, **k: filas)
...
def test_sin_consulta_configurada_no_se_toca_el_erp(monkeypatch):
    ...
    monkeypatch.setattr(consultas, "SQL_AUSENCIAS", "")

    def no_deberia_llamarse(*a, **k):
        raise AssertionError("no hay que consultar el ERP si la consulta esta vacia")

    monkeypatch.setattr(lecturas, "consultar", no_deberia_llamarse)
    assert lecturas.leer_ausencias(DIA) == {}
...
def test_una_parcial_dice_como_afecta_a_la_jornada(monkeypatch):
    ...
    parcial = lecturas.leer_ausencias(DIA)["15"]
...
def test_las_cuatro_formas_de_una_parcial():
    ...
    assert lecturas.cuando_falta("07:00", "10:00") == "entra a las 10:00"
    assert lecturas.cuando_falta("13:00", "15:00") == "se marcha a las 13:00"
    assert lecturas.cuando_falta("09:00", "11:00") == "fuera de 09:00 a 11:00"
    # Marcada como parcial pero de apertura a cierre: no es parcial de verdad.
    assert lecturas.cuando_falta("07:00", "15:00") is None
    assert lecturas.cuando_falta(None, "10:00") is None
...
def test_una_parcial_que_cubre_la_jornada_es_dia_completo(monkeypatch):
    ...
    assert lecturas.leer_ausencias(DIA)["9"]["parcial"] is False
...
def test_una_baja_larga_conserva_su_rango(monkeypatch):
    ...
    victor = lecturas.leer_ausencias(DIA)["31"]
...
def test_un_motivo_vacio_no_deja_la_fila_muda(monkeypatch):
    ...
    assert [a["motivo"] for a in lecturas.leer_ausencias(DIA).values()] == ["Ausencia", "Ausencia"]
...
def test_la_consulta_no_lee_el_motivo_escrito_en_el_portal():
    ...
    assert "Reason" not in lecturas.consultas.SQL_AUSENCIAS
    ...
    for etiqueta in ("'De baja'", "'Vacaciones'", "'Ausencia'"):
        assert etiqueta in lecturas.consultas.SQL_AUSENCIAS
...
def test_si_el_erp_falla_el_gantt_se_pinta_igual(monkeypatch):
    ...
    monkeypatch.setattr(lecturas, "consultar", denegado)
    assert lecturas.leer_ausencias(DIA) == {}
...
def test_en_la_vista_de_maquinas_no_hay_ausencias(monkeypatch):
    ...
    monkeypatch.setattr(lecturas, "leer_abiertas", lambda *a: [])
    monkeypatch.setattr(lecturas, "leer_cola", lambda *a: [])
    assert produccion.calcular_avisos(vista="maquina", dia=DIA) == {"sin_salida": {}, "ausencias": {}}
